...
namespace Assign14.Controllers
{
    public class PlayersController : Controller
    {
        ...
        // POST: Players/Edit/5
        [HttpPost]
        public ActionResult Edit(int PId, Player player)
        {
            try
            {
                con = new SqlConnection(conString);
                cmd = new SqlCommand("update Player set FirstName=@FName,LastName=@LName,JerseyNumber=@JerseyNumber,Position=@Position,Team=@Team where PId=@PId");
                cmd.Connection = con;
                cmd.Parameters.AddWithValue("@PId", player.PId);
                cmd.Parameters.AddWithValue("@FName", player.FName);
                cmd.Parameters.AddWithValue("@LName", player.LName);
        ...
        public ActionResult Delete(int PId, Player player)
        {
            ...
            {
                con = new SqlConnection(conString);
                cmd = new SqlCommand("delete from Players where PId=@PId");
                cmd.Connection = con;
                cmd.Parameters.AddWithValue("@PId", player.PId);
